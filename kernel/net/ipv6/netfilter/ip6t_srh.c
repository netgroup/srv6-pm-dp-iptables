...
static bool srh_list_match(const struct sk_buff *skb,
			   const struct ipv6_sr_hdr *srh, int srhoff,
			   const struct ip6t_srh2 *srhinfo);
...
static bool srh2_mt6(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (srhinfo->mt_flags & IP6T_SRH_SID_LIST)
		if (NF_SRH_INVF(srhinfo, IP6T_SRH_INV_SID_LIST,
				!srh_list_match(skb, srh, srhoff, srhinfo)))
			return false;
			   ...
			   const struct ip6t_srh2 *srhinfo)
{
	...
	listoff = srhoff + sizeof(*srh);

	for(i = 0, k = srh->first_segment;
	    i < srh->first_segment + 1; ++i, --k) {
		list_addr = skb_header_pointer(skb, listoff +
					       (i * sizeof(_list_addr)),
					       sizeof(_list_addr), &_list_addr);

		if (ipv6_masked_addr_cmp(list_addr,
					 &srhinfo->sid_list_msk[k],
					 &srhinfo->sid_list_addr[k]))
			return false;
	}
...
MODULE_AUTHOR("Andrea Mayer <andrea.mayer@gmail.com>");
